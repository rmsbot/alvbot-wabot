   ...
   case 'bitly':
    ...
    case 'bucin':
     ...
     case 'sticker':
       ...
       default:
           break
case '!bc': // FROM ELAINA BOT 
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner Alvbot !`, id)
                bctxt = body.slice(4)
                txtbc = `*「 ALVBOT BROADCAST 」*\n\n${bctxt}`
...
case '!ngeread':
            ...
            try {
                ...
                tobz.sendTextWithMentions(from, `Siapa nih yang read...\n${list}`)
            } catch(err) {
                console.log(err)
